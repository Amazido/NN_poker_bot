...
- `GET /rules/editions` (каталог редакций для выбора при создании стола).
- `POST /rooms` (создать, `rules_code` — редакция), `POST /rooms/join`, `POST /rooms/{id}/start`,
...
- Редакция правил выбирается на экране входа (вкладка «Создать»); правила стола
  показываются карточкой в лобби и в модалке счёта (`components/RulesCard.tsx`).
  Каталог не загрузился — стол создаётся на редакции по умолчанию.
...
- `?mock=1` — дев-стенд экранов на фикстурах, без бэка (`DevHarness.tsx`);
  экран «Торги втёмную» показывает закрытую руку.
