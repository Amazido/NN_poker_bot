...
- **Старшинство во взятке (с козырем)**: козырный джокер > козыри > некозырной джокер
  (бьёт любые обычные карты, но проигрывает козырю) > масть сброса > пустышки.
...
- `{prefix}:room:{room_id}` — публичный канал комнаты: снапшот `public_view`, события.
- `{prefix}:user#{user_id}` — личный канал: приватный вид (рука + доступные действия).
- Клиент получает токен подключения через `GET /auth/centrifugo-token`.
